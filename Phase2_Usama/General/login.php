...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $username = mysqli_real_escape_string($conn, $_POST['username']); // Get the username they typed
  $password = mysqli_real_escape_string($conn, $_POST['password']); // Get the password they typed

  if (empty($username) || empty($password)) {
    $error = "Please type your username and password!";
  } else {
    // First, check if they are a staff member
    $staff_sql = "SELECT sid, spassword, sname, 'staff' AS role FROM staff WHERE sname = '$username'";
    ...
    if ($staff_result && mysqli_num_rows($staff_result) > 0) {
      ...
      if ($password === $user['spassword']) { // Check if the password is correct
        ...
        $error = "Wrong password for staff!";
      }
    } else {
      // If not staff, check if they are a customer
      $customer_sql = "SELECT cid, cpassword, cname, 'customer' AS role FROM customer WHERE cname = '$username'";
      $customer_result = mysqli_query($conn, $customer_sql);
      if ($customer_result && mysqli_num_rows($customer_result) > 0) {
        $user = mysqli_fetch_assoc($customer_result); // Get their details
        // echo "Found customer: " . $user['cname'] . "<br>"; // Remove or move this after session set
        if ($password === $user['cpassword']) { // Check if the password is correct
          ...
          $error = "Wrong password for customer!";
        }
      } else {
        $error = "User not found with username: $username"; // Show if we can’t find them
      }
    }
  }
}
