...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_order'])) {
    $oid = mysqli_real_escape_string($conn, $_POST['delete_order']);
    ...
    if (mysqli_num_rows($order_result) > 0) {
        ...
        if (!$deliver_date || ($deliver_date && $current_date <= $two_days_before)) {
            ...
            if (mysqli_query($conn, $delete_order)) {
                // Update material reserved quantities (mrqty)
...
$orders_list = "SELECT oid, odate, pid, oqty, ocost, odeliverdate, ostatus 
                FROM orders WHERE cid = '$cid' ORDER BY odate DESC";
        ...
        <?php if (mysqli_num_rows($orders_result) > 0) { ?>
            <form method="post" action="">
                <table>
                    ...
                    <?php
                    mysqli_data_seek($orders_result, 0); // Reset pointer if needed
